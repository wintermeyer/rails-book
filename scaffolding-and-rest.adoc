...
xref:first-steps-with-rails.adoc#first-steps-with-rails["First Steps with Rails"] and
xref:activerecord.adoc#activerecord["ActiveRecord"], so you will be able to
...
a redirect (see xref:scaffolding-and-rest.adoc#redirects[section Redirects]).
...
the chapter xref:routing.adoc#routing["Routes"]), a controller and several views for us.
