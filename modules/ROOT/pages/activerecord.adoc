...
[NOTE]
====
The examples in this chapter were tested against Rails 8.1 and
Ruby 4.0. A handful of Active Record features are worth knowing
about early, even if we only use the most common ones in this
chapter:

- `normalizes :email, with: \->(e) { e.strip.downcase }`,
  declarative attribute normalisation at the model level.
- `has_secure_password` supports `reset_token` and email
  confirmation workflows out of the box (Rails 8's authentication
  generator uses it).
- Encrypted attributes: `encrypts :social_security_number`.
- Composite primary keys.
- `insert_all` / `upsert_all` for bulk writes.

These are additions, not replacements, so the fundamentals we
cover below still form the mental model you need.
====
...
[source,bash]
----
$ bin/rails generate model Country name population:integer
...
NOTE: You will find more details on migrations in
      xref:activerecord.adoc#migrations[the section "Migrations"].
...
[[ar-getters-and-setters]]
=== Getters and Setters

To read and write values of a SQL table row you can use getters and setters by
ActiveRecord provided link:https://wintermeyer-consulting.de/ruby/book/methods-once-again.html#getters-and-setters[getters and setters].
...
[source,bash]
----
$ bin/rails generate model product name 'price:decimal{7,2}'
  [...]
$
----
...
In xref:activerecord.adoc#migrations["Migrations"] we will provide more
...
TIP: For already existing records, you can also check for changes with
     the method `changed?` (see xref:activerecord.adoc#changed["changed?"]).
...
when it has to do it (it's called xref:activerecord.adoc#lazy-loading[Lazy Loading]).
...
link:https://wintermeyer-consulting.de/ruby/book/loops.html#iterators[iterators] and link:https://wintermeyer-consulting.de/ruby/book/collections.html#iterator-each[each],
...
xref:activerecord.adoc#create[create] all initial records in a new Rails application.
...
$ rails new jukebox
  [...]
$ cd jukebox
$ bin/rails generate model Album name release_year:integer
  [...]
$ bin/rails db:migrate
  [...]
----
...
You can also use `where` to search for link:https://wintermeyer-consulting.de/ruby/book/ranges.html#range[ranges]:
...
xref:activerecord.adoc#searching-and-finding-with-queries[section "Searching and
...
xref:activerecord.adoc#simple-editing[the section "Simple Editing"]:
...
$ rails new shop
  [...]
$ cd shop
$ bin/rails generate model Product name 'price:decimal{8,2}'
...
[source,bash]
----
$ bin/rails generate model Category name
  [...]
$
----
...
[source,bash]
----
$ bin/rails generate model product name price:integer category:references
      invoke  active_record
...
xref:activerecord.adoc#joins[joins]). Again, you can use it to search within a 1:n
...
xref:activerecord.adoc#deletedestroy-a-record["Delete/Destroy a Record"]. In the
...
IMPORTANT: Please always remember the difference between the methods
           `destroy` (see xref:activerecord.adoc#destroy["destroy"]) and `delete`
           (see xref:activerecord.adoc#delete[the "delete"]). This association only
...
[source,bash]
----
$ bin/rails generate model order delivery_address
----
...
[source,bash]
----
$ bin/rails generate model line_item order:references \
...
section xref:activerecord.adoc#has_many-1n-association["has_many - 1:n Association"].
...
[source,bash]
----
$ bin/rails generate model Car name
  [...]
$ bin/rails generate model Bike name
  [...]
$ bin/rails generate model Tag name taggable:references{polymorphic}
  [...]
$ bin/rails db:migrate
  [...]
----
...
normal xref:activerecord.adoc#has_many-1n-association[has_many association].
...
$ rails new bookshelf
  [...]
$ cd bookshelf
$ bin/rails generate model book title
  [...]
$ bin/rails generate model author book:references first_name last_name
  [...]
$ bin/rails db:migrate
  [...]
$
...
xref:activerecord.adoc#destroy["destroy"]). The record is already
...
This works with the method `build` (see xref:activerecord.adoc#build["build"]) and
`create` (see xref:activerecord.adoc#create["create"]).
...
NOTE: For all error messages, please note the chapter
      xref:i18n.adoc#i18n[Internationalization].
...
`exclusion` is the inversion of xref:activerecord.adoc#inclusion[inclusion].
...
TIP: In real applications you should use Ruby's built-in
     `URI::MailTo::EMAIL_REGEXP` instead of hand-rolling your own
     regex: `format: { with: URI::MailTo::EMAIL_REGEXP }`.
...
$ rails new my_hotel
  [...]
$ cd my_hotel
$ bin/rails generate model reservation \
...
.Prompt pattern: migration + model + fixtures + test in one go
[sidebar]
****
A schema change typically touches four files: the migration,
the model, the fixtures, and at least one test. Doing all four
in a single agent round-trip is much less error-prone than
splitting them.

Prompt for Claude Code (or equivalent):

[quote]
____
Add a `status` string column to `Order` with default
"pending" and an index. Define `enum :status, %w[pending
paid refunded]` on the model. Update
`test/fixtures/orders.yml` so every fixture has an explicit
status. Write a model test per enum transition. Then run
`bin/rails db:migrate && bin/rails test`.
____

Verify: run the migration yourself, check `db/schema.rb`
matches the intent, re-run `bin/rails test`. Never let an
agent hand-edit `db/schema.rb` — that file is the output of
migrations, not the input. See
...
[source,bash]
----
$ bin/rails generate migration addHeightToProduct height:integer
      invoke  active_record
...
----
class AddHeightToProduct < ActiveRecord::Migration[8.1]
  def change
...
[source,bash]
----
$ bin/rails generate migration create_index
      invoke  active_record
      create    db/migrate/20151217190442_create_index.rb
$
----

In the file `db/migrate/20151217190442_create_index.rb` we create the
index with `add_index` in the method `self.up`, and in the method
`self.down` we delete it again with `remove_index`:

[source,ruby]
.db/migrate/20151217190442_create_index.rb
----
class CreateIndex < ActiveRecord::Migration[8.1]
  def up
...
[source,bash]
----
$ bin/rails generate model product name:string:index
...
$ rails new shop
  [...]
$ cd shop
$ bin/rails generate model user email login
  [...]
$ bin/rails db:migrate
  [...]
----
...
In the section xref:activerecord.adoc#default-values["Default Values"] you will find
...
$ rails new shop
  [...]
$ cd shop
$ bin/rails generate model order product_id:integer quantity:integer
  [...]
$ bin/rails db:migrate
  [...]
----
