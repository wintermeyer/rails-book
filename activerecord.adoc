...
NOTE: The examples in this chapter were tested against Rails 8.1
      and Ruby 4.0. A handful of Active Record APIs gained useful
      companions since Rails 5.2; the most important ones you'll
      want to know about are:

      - `normalizes :email, with: -> e { e.strip.downcase }` —
        declarative attribute normalisation at the model level.
      - `has_secure_password` now supports `reset_token` / email
        confirmation workflows out of the box (Rails 8's
        authentication generator uses it).
      - Encrypted attributes:
        `encrypts :social_security_number`.
      - Composite primary keys.
      - `insert_all` / `upsert_all` for bulk writes.

      They are all "in addition to" rather than "instead of" what
      we cover below, so an old Rails 5.2 book chapter mostly
      still teaches you the right mental model.
